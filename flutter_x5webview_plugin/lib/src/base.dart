...
const _kChannel = 'flutter_x5webview_plugin';
...
class FlutterX5webviewPlugin {
  factory FlutterX5webviewPlugin() => _instance ??= FlutterX5webviewPlugin._();

  FlutterX5webviewPlugin._() {
    _channel.setMethodCallHandler(_handleMessages);
  }

  static FlutterX5webviewPlugin _instance;

  final _channel = const MethodChannel(_kChannel);

  final _onBack = StreamController<Null>.broadcast();
  final _init = StreamController<Null>.broadcast();
  ...
  final _javascriptChannelMessage =
  StreamController<JavascriptChannel>.broadcast();
  ...
  Future<Null> _handleMessages(MethodCall call) async {
    switch (call.method) {
      ...
      case "javascriptChannelMessage":
        _javascriptChannelMessage
            .add(JavascriptChannel.fromJson(call.arguments));
        break;
      case 'onState':
        _onStateChanged.add(
  ...
  Stream<JavascriptChannel> get onJavascriptChannelMessage =>
      _javascriptChannelMessage.stream;
  ...
  Future<bool> canGoBack() async =>
      await _channel.invokeMethod<bool>('canGoBack');
  ...
  Future<bool> canGoForward() async =>
      await _channel.invokeMethod<bool>('canGoForward');
  ...
  Future<bool> setDownloadWithoutWifi(bool isDownloadWithoutWifi) async {
    if (Platform.isAndroid) {
